...
export default function FundUsdWithKesMpesa() {
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.metadata.auth_url) {
        return (window.location.href = data.metadata.auth_url);
      }
  ...
  const selectedMethod = useMemo(() => {
    ...
    return availableOptions.find(
      (m) => m.channel === "mobile_money" && m.network_code === "mpesa"
    );
  }, [ioMethods]);
